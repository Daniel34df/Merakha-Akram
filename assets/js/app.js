...
(function (root) {
      ...
      lignes.push([
        ...
        e.dernierPassage
          ? util.formatJour(e.dernierPassage.slice(0, 10)) +
            ...
            (domi.libelleMoyen(e.dernierMoyen) ? ' — ' + domi.libelleMoyen(e.dernierMoyen) : '') +
  ...
  async function renderJournal() {
    ...
    carte.hidden = S.mode !== 'serveur' || ui.sessionManquante();
    if (carte.hidden) return;
    try {
  ...
  /* app.js s'inscrit comme les autres : les écrans lui demandent un rendu
     général ou un changement d'onglet, et le font par le même carnet. */
  /* En attendant que le registre ait son propre module, c'est app.js qui
     l'inscrit : la domiciliation lui demande la fiche d'un destinataire et le
     remplissage du sélecteur de langues. Ces deux lignes déménageront avec lui,
     et le carnet ne changera pas. */
  ui.inscrire('registre', {
  ...
  async function start() {
    ...
    /* Les écrans sortis dans leur propre module se branchent ici : au
       chargement de leur fichier, ils ne peuvent pas encore s'adresser les uns
       aux autres — l'ordre des balises <script> ne le garantit pas. */
    Object.keys(ecrans).forEach(function (nom) {
      if (typeof ecrans[nom].init === 'function') ecrans[nom].init();
    });
    fillSettingsForm();
    syncCopiesFromSettings(true);
    renderAll();
